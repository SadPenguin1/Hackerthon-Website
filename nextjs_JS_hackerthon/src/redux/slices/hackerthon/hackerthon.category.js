...
import { findCategoryAPI, getCategoryByIdAPI } from "../../../service/hackerthon/hackerthon.category.service";
...
export function getCategories() {
    return async (dispatch, getState) => {
        ...
        const { hackerthonCategory } = getState();

        const resp = await findCategoryAPI({ ...hackerthonCategory.search, value: `%${hackerthonCategory.search.value}%` });
        console.log(resp);
       if (resp.code === '200')
...
export function getCategory(id) {
    return async (dispatch) => {
        ...
        if (resp.code === '200')
            dispatch(slice.actions.setCategory(resp));
        else
            dispatch(slice.actions.hasError(resp));
    };
}