...
import { deleteTagExerciseAPI, deleteTagExercisesAPI, findTagExercisesAPI } from '../../../service/hackerthon/hackerthon.tagExercise.service';
// sections
...
import TagExerciseNewForm from "./TagExerciseNewForm";

export default function TagExerciseList({ item }) {
  const { translate } = useLocales();
  const { themeStretch } = useSettings();
  const { enqueueSnackbar } = useSnackbar();

  const [data, setData] = useState({
    tagExercises: [], totalElements: 0, numberOfElements: 0, error: null, isLoading: false,
    search: { page: 0, size: 10, value: '', orders: [{ order: "desc", property: 'id' }], filterBys: { "tag.id": item?.id } },
  });
  const { tagExercises, totalElements, numberOfElements, search, error } = data;

  const TABLE_HEAD = [
    { id: 'id', label: translate('hackerthon.tagExercise.id'), alignRight: false, checked: false, sort: true },
    { id: 'orderNo', label: translate('hackerthon.tagExercise.orderNo'), alignRight: false, checked: true, sort: true },
    { id: 'exercise', label: translate('hackerthon.tagExercise.exercise'), alignRight: false, checked: true, sort: false },
  ...
  const getTagExercises = async () => {
    setData({ ...data, isLoading: true });
    const resp = await findTagExercisesAPI({ ...search, value: `%${search.value}%` });
    if (resp.code === '200')
      setData({ ...data, tagExercises: resp.data, totalElements: resp.totalElements, numberOfElements: resp.numberOfElements, isLoading: false, error: null });
    else
  ...
  const handleRequestSort = (property) => {
    ...
    setData({ search: { ...search, orders: [{ order, property }] }, tagExercises, totalElements, numberOfElements, error: null });
  };

  const handleSelectAllClick = (checked) => {
    if (checked) {
      const selected = tagExercises.map((n) => n.id);
  ...
  const handleChangeRowsPerPage = (event) => {
    setData({ search: { ...search, page: 0, size: parseInt(event.target.value, 10) }, tagExercises, totalElements, numberOfElements, error: null });
  };

  const handleChangePage = (page) => {
    setData({ search: { ...search, page }, tagExercises, totalElements, numberOfElements, error: null });
  };

  const handleFilterByName = (value) => {
    setData({ search: { ...search, value }, tagExercises, totalElements, numberOfElements, error: null });
  };
  ...
  const confirmDeleteSeriesPost = async () => {
    let resp;
    if (selected.length > 0) resp = await deleteTagExercisesAPI(selected);
    else resp = await deleteTagExerciseAPI(selectedId);
  ...
  return (
    <Container maxWidth={themeStretch ? false : 'lg'}>
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        <Box sx={{ flexGrow: 1 }}>
          <Typography variant="h4" gutterBottom>
            {translate('hackerthon.tagExercise.listTagExercise')}
          </Typography>
        </Box>
      </Box>
      ...
      <Card>
        <TagExerciseNewForm tag={item} refresh={getTagExercises} />
                  ...
                  return (
                    ...
                    >
                      ...
                      {TABLE_HEAD.map((head) => {
                        ...
                        if (head.id === 'tag') return <TableCell key={head.id}>{row[head.id]?.title}</TableCell>;
                        if (head.id === 'exercise')
