...
import { deleteExamExerciseAPI, deleteExamExercisesAPI, findExamExercisesAPI } from '../../../service/hackerthon/hackerthon.examExercise.service';
// sections
...
export default function ExamExerciseList({ item }) {
  const { translate } = useLocales();
  const { themeStretch } = useSettings();
  const { enqueueSnackbar } = useSnackbar();

  const [data, setData] = useState({
    examExercises: [], totalElements: 0, numberOfElements: 0, error: null, isLoading: false,
    search: { page: 0, size: 10, value: '', orders: [{ order: "desc", property: 'id' }], filterBys: { "exam.id": item?.id } },
  });
  const { examExercises, totalElements, numberOfElements, search, error } = data;

  const TABLE_HEAD = [
    { id: 'id', label: translate('hackerthon.examExercise.id'), alignRight: false, checked: false, sort: true },
    { id: 'orderNo', label: translate('hackerthon.examExercise.orderNo'), alignRight: false, checked: true, sort: true },
    { id: 'exercise', label: translate('hackerthon.examExercise.exercise'), alignRight: false, checked: true, sort: false },
  ...
  const getExamExercises = async () => {
    setData({ ...data, isLoading: true });
    const resp = await findExamExercisesAPI({ ...search, value: `%${search.value}%` });
    if (resp.code === '200')
      setData({ ...data, examExercises: resp.data, totalElements: resp.totalElements, numberOfElements: resp.numberOfElements, isLoading: false, error: null });
    else
  ...
  const handleRequestSort = (property) => {
    ...
    setData({ search: { ...search, orders: [{ order, property }] }, examExercises, totalElements, numberOfElements, error: null });
  };

  const handleSelectAllClick = (checked) => {
    if (checked) {
      const selected = examExercises.map((n) => n.id);
  ...
  const handleChangeRowsPerPage = (event) => {
    setData({ search: { ...search, page: 0, size: parseInt(event.target.value, 10) }, examExercises, totalElements, numberOfElements, error: null });
  };

  const handleChangePage = (page) => {
    setData({ search: { ...search, page }, examExercises, totalElements, numberOfElements, error: null });
  };

  const handleFilterByName = (value) => {
    setData({ search: { ...search, value }, examExercises, totalElements, numberOfElements, error: null });
  };
  ...
  const confirmDeleteSeriesPost = async () => {
    let resp;
    if (selected.length > 0) resp = await deleteExamExercisesAPI(selected);
    else resp = await deleteExamExerciseAPI(selectedId);
  ...
  return (
    <Container maxWidth={themeStretch ? false : 'lg'}>
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        <Box sx={{ flexGrow: 1 }}>
          <Typography variant="h4" gutterBottom>
            {translate('hackerthon.examExercise.listExamExercise')}
          </Typography>
        </Box>
      </Box>
      ...
      <Card>
        <ExamExerciseNewForm exam={item} refresh={getExamExercises} />
