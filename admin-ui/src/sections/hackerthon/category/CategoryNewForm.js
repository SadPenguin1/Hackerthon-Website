...
import { createCategoryAPI, updateCategoryAPI } from '../../../service/hackerthon/hackerthon.category.service';
...
export default function CategoryNewForm({ isEdit, isView, currentItem }) {
  ...
  const onSubmit = async (data) => {
    ...
    if (resp.code === '200') {
      ...
      navigate(PATH_DASHBOARD.hackerthon.categories);
    } else
    ...
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Grid container spacing={3}>
        <Grid item xs={12} md={7}>
          <Card sx={{ p: 3 }}>
            <Stack spacing={3}>
              <RHFTextField name="title" label={translate('hackerthon.category.title')} disabled={isView} />
              <RHFTextField name="orderNo" label={translate('hackerthon.category.orderNo')} disabled={isView}
              ...
              />
              <div>
                <LabelStyle>{translate('hackerthon.category.description')}</LabelStyle>
          ...
          <Stack spacing={3}>
            <Card sx={{ p: 3 }}>
              <Stack spacing={3} mt={2}>
                ...
                <RHFTextField name="metaTitle" label={translate('hackerthon.category.metaTitle')} disabled={isView} />
                <RHFTextField name="slug" label={translate('hackerthon.category.slug')} disabled={isView} />
                <RHFTextField name="metaDescription" label={translate('hackerthon.category.metaDescription')} multiline rows={4} disabled={isView} />
                <RHFTextField name="createdAt" label={translate('hackerthon.category.createdAt')} disabled={isView} />
            ...
            </Card>
            {
              isView ? (<Button
                ...
                to={`${PATH_DASHBOARD.hackerthon.root}/category/${currentItem?.id}/edit`}
                size="large"
