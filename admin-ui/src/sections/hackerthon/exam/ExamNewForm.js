...
import { createExamAPI, updateExamAPI } from '../../../service/hackerthon/hackerthon.exam.service';
...
export default function ExamNewForm({ isEdit, isView, currentItem }) {
  ...
  const onSubmit = async (data) => {
    ...
    if (resp.code === '200') {
      ...
      navigate(PATH_DASHBOARD.hackerthon.exams);
          ...
          <Stack spacing={3}>
            <Card sx={{ p: 3 }}>
              <Stack spacing={3} mt={2}>
                <RHFTextField name="title" label={translate('hackerthon.exam.title')} disabled={isView} />
                <div>
                  <LabelStyle>{translate('hackerthon.exam.description')}</LabelStyle>
          ...
          <Stack spacing={3}>
            <Card sx={{ p: 3 }}>
              <Stack spacing={3} mt={2}>
                ...
                <RHFTextField name="metaTitle" label={translate('hackerthon.exam.metaTitle')} disabled={isView} />
                <RHFTextField name="slug" label={translate('hackerthon.exam.slug')} disabled={isView} />
                <RHFTextField name="metaDescription" label={translate('hackerthon.exam.metaDescription')} multiline rows={4} disabled={isView} />
              </Stack>
            </Card>
            {isView ? (
              <Button
                ...
                to={`${PATH_DASHBOARD.hackerthon.root}/exam/${currentItem?.id}/edit`}
                size="large"
